...
from health_management.models import [
    User, Patient, MedicalPractitioner, Appointment, Rating
]
...
class APITests(TestCase):
    ...
    def test_login_success(self):
        response = self.client.post(
            reverse('login'), {
                'email': 'patient@example.com', 'password': 'test123'}, format='json')
    ...
    def test_login_invalid_credentials(self):
        response = self.client.post(
            reverse('login'), {
                'email': 'wrong@example.com', 'password': 'wrong'}, format='json')
    ...
    def test_register_patient_success(self):
        ...
        response = self.client.post(
            reverse('register_patient'), data, format='json')
    ...
    def test_rate_practitioner_as_patient(self):
        ...
        response = self.client.post(
            reverse('rate_practitioner'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
